...
def order(call):
    keyboard = types.InlineKeyboardMarkup()
    keyboard.row(types.InlineKeyboardButton(text="Yes", callback_data="Yes"), types.InlineKeyboardButton(text="No", callback_data="No"))
    bot.send_message(call.message.chat.id, f"Do you want to make money?", reply_markup=keyboard)

def job(call):
    bot.send_message(call.message.chat.id, f"Okay, vot pervoe zadanie: asdasdasdasdasdasdassdasdasdsad")
    @bot.message_handler(func=lambda message: True, content_types=['text'])
    def first_quest(message):
        if message.text == "Answer":
            sleep(3)
            bot.send_message(call.message.chat.id, f"Well done, take 2500$")
            return second_quest(message)
        else:
            bot.send_message(call.message.chat.id, f"Wrong answer!")

@bot.message_handler(func=lambda message: True, content_types=['text'])
def second_quest(message):
    bot.send_message(message.from_user.id, f"Okay, vot vtoroe zadanie:")
    if message.text == "Lalala":
        bot.send_message(message.from_user.id, f"Well done, take 2500$")
    else:
        bot.send_message(message.from_user.id, f"Wrong answer!")
